  ...
  private boolean isLineLitByLamp(int lampR, int lampC, int r, int c, Puzzle puzzle) {
    if (lampR == r) {
      for (int col = Math.min(lampC, c) + 1; col < Math.max(lampC, c); col++) {
        if (puzzle.getCellType(r, col) == CellType.WALL
            || puzzle.getCellType(r, col) == CellType.CLUE) {
          return false;
        }
      }
    } else if (lampC == c) {
      for (int row = Math.min(lampR, r) + 1; row < Math.max(lampR, r); row++) {
        if (puzzle.getCellType(row, c) == CellType.WALL
            || puzzle.getCellType(row, c) == CellType.CLUE) {
          return false;
        }
      }
    }
